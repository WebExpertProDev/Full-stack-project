...
export const MoreFilters: React.FunctionComponent<IMoreFilters.IProps> = ({
  setfilterOptions
}) => {
  ...
  useEffect(() => {
    const trueOptions = options.filter(op => op.status == true);

    setfilterOptions("utilities", options);
    setfilterOptions("minPrice", minPrice);
    setfilterOptions("maxPrice", maxPrice);
